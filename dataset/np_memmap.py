...
class NPDataset(Dataset):
    def __init__(self, _path, seq_length=1024, train: bool = True, dtype=np.uint16):
        file_paths = glob.glob(_path)
        if train: file_paths = file_paths[:-1] if len(file_paths) > 1 else file_paths
    ...
    def __len__(self) -> int: return self.total_size

    def __getitem__(self, index) -> dict:
        if index < 0: index += self.total_size
        if index >= self.total_size or index < 0: raise IndexError("Index out of range")

        file_index = np.searchsorted(self.cumulative_sizes, index, side='right') - 1
        internal_index = index - self.cumulative_sizes[file_index]

        item = torch.from_numpy(np.int64(self.memmaps[file_index][internal_index]))
...
class NPDataModule(L.LightningDataModule):
    ...
    def train_dataloader(self): return DataLoader(self.train_ds, batch_size=self.batch_size, num_workers=self.num_workers)
